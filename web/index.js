...
app.get("/api/getSubscription", async (_req, res) => {
  const session =  res.locals.shopify.session
  const data = await subscriptiondetail(session)
...
app.get('/api/checkout/enableblocks',async (_req,res)=>{
  const session =  res.locals.shopify.session;
  const data = await enablecheckoutblock(session)
  res.status(200).send(data)
})
